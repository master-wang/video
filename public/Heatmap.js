  ...
  var f = (function k() {
    ...
    function n() {
      ...
      if (q.plugin) {
        ...
        if (!c.plugins[s]) {
          throw new Error(
            "Plugin '" + s + "' not found. Maybe it was not registered."
          );
        } else {
          var r = c.plugins[s];
...
var BMapLibHeat = (window.BMapLibHeat = BMapLibHeat || {});
(function () {
  var a = (BMapLibHeat.HeatmapOverlay = function (c) {
    this.conf = c;
  ...
  a.prototype.draw = function () {
    ...
    if (this.latlngs.length > 0) {
      ...
      if (this.conf.radiusChangeByZoom) {
        this.heatmap._store._cfgRadius = this.conf.radiusChangeByZoom(
          this._map.getZoom()
        );
      }
